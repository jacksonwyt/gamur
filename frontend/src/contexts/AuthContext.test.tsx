...
describe('AuthContext', () => {
  ...
  it('login should store token and update state and apiClient headers', async () => {
    ...
    expect(apiClient.defaults.headers.common['Authorization']).toBe(`Bearer ${newToken}`);
  });
  ...
  it('logout should remove token and update state and apiClient headers', async () => {
   ...
   it('should handle error when loading token fails', async () => {
    (SecureStore.getItemAsync as jest.Mock).mockRejectedValueOnce(new Error('Failed to load'));
    ...
    expect(result.current.isLoading).toBe(false);
    expect(result.current.token).toBeNull();
    expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to load auth token from storage', expect.any(Error));
    consoleErrorSpy.mockRestore();
  });

  it('should handle error when saving token fails during login', async () => {
    (SecureStore.setItemAsync as jest.Mock).mockRejectedValueOnce(new Error('Failed to save'));
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    const { result } = renderHook(() => useAuth(), { wrapper });
  ...
  it('should handle error when deleting token fails during logout', async () => {
    (SecureStore.deleteItemAsync as jest.Mock).mockRejectedValueOnce(new Error('Failed to delete'));
    const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    const { result } = renderHook(() => useAuth(), { wrapper });
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to delete auth token', expect.any(Error));
    consoleErrorSpy.mockRestore();
  });
}); 