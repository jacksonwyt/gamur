...
type HabitListScreenProps = NativeStackScreenProps<RootStackParamList, 'HabitList'>;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      <Appbar.Header>
        ...
        <Appbar.Action icon="plus" onPress={() => navigation.navigate('CreateHabit')} />
      </Appbar.Header>
      <View style={styles.content}>
        {dummyHabits.length === 0 ? (
          <Text style={styles.emptyText}>No habits added yet. Tap '+' to create one!</Text>
        ) : (
          <List.Section>
            ...
            {dummyHabits.map((habit) => (
              <List.Item
                ...
                right={(props) => (
                  <List.Icon
                    {...props}
                    icon={habit.completed ? 'check-circle' : 'checkbox-blank-circle-outline'}
                  />
